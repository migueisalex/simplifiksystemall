...
interface LowResolutionAlertModalProps {
  onClose: () => void;
  onKeep: () => void;
  onEnhance: () => void;
}

const LowResolutionAlertModal: React.FC<LowResolutionAlertModalProps> = ({ onClose, onKeep, onEnhance }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[70] p-4" onClick={onClose}>
      <div className="bg-white dark:bg-dark-card rounded-lg shadow-2xl w-full max-w-md flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          <div className="sm:flex sm:items-start">
             <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100 dark:bg-yellow-900/50 sm:mx-0 sm:h-10 sm:w-10">
                <svg className="h-6 w-6 text-yellow-600 dark:text-yellow-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" aria-hidden="true">
             ...
             <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500 dark:text-gray-300">
                    A imagem selecionada tem baixa resolução e pode parecer ruim no post final. Você pode usar IA para recriá-la em alta qualidade.
                  </p>
                </div>
              </div>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-lg">
          <button
            type="button"
            className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-dark-card sm:ml-3 sm:w-auto sm:text-sm transition"
            onClick={onEnhance}
          >
            Melhorar com IA
          </button>
...
export default LowResolutionAlertModal;
